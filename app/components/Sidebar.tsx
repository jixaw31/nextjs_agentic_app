...
interface SidebarProps {
  ...
  setMessages: (messages: any[]) => void; // You can type messages more specifically if you have a message model
...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const { input, setInput, messages,
          ...
          fetchConversations,
          isMinimized, setIsMinimized} = useChat();
        ...
        >
          ...
          <div
            className={`
              ...
              ${isMinimized ? 'overflow-hidden' : 'overflow-y-auto max-h-[calc(100vh-200px)] pr-1 dark-scrollbar'}
            `}
          >            
            {conversations.length > 0 ? (
              conversations.map((conv) => (
                <div
                  ...
                  className={`p-2 rounded cursor-pointer mb-2 text-white transition-all
                    ${!isMinimized && conversationId === conv.id ? ' bg-gray-500/90' : ''}
              ...
              !isMinimized && (
                <div className="text-white text-sm italic mt-4">
